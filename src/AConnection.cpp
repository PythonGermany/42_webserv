...
// + std::strerror(errno)); 	pid = fork(); 	if (pid == -1) 		throw
// std::runtime_error(std::string("AConnection::runCGI(): ") +
// std::strerror(errno)); 	if (pid == 0)
// 	{
// 		if (dup2(pipefd[1], STDOUT_FILENO) == -1)
// 		{
// 			std::cerr << "webserv: error: dup2(): " <<
// std::strerror(errno) << std::endl; 			exit(EXIT_FAILURE);
// 		}
// 		close(pipefd[0]);
// 		close(pipefd[1]);
...
// 		for (std::vector<std::string>::iterator it = arg.begin(); it !=
// arg.end(); ++it) 			c_arg.push_back(const_cast<char *>(it->c_str()));
// 		c_arg.push_back(NULL);
// 		for (std::vector<std::string>::iterator it = env.begin(); it !=
// env.end(); ++it) 			c_env.push_back(const_cast<char *>(it->c_str()));
// 		c_env.push_back(NULL);
// 		execve(program.c_str(), c_arg.data(), c_env.data());
// 		std::cerr << "webserv: error: execve(): " <<
// std::strerror(errno) << std::endl; 		exit(EXIT_FAILURE);
// 	}
...
void AConnection::onNoPollIn(struct pollfd &pollfd) {
  ...
  if (pollfd.events & POLLOUT) return;
