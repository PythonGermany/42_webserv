...
void Poll::add(struct pollfd &src) { getInstance().pollfds.push_back(src); }
...
// void Poll::_checkTimeout(struct pollfd &pollfd, AConnection *conn, int
// &timeout)
// {
...
void Poll::iterate() {
  ...
  for (size_type i = 0; i < size;) {
    if (pollfds[i].revents & POLLOUT) callbackObjects[i]->onPollOut(pollfds[i]);
