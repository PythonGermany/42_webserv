...
void AConnection::send(std::string msg)
{
	if (_writeBuffer.empty())
		Poll::addPollEvent(POLLOUT, this);
	_writeBuffer += msg;
}
...
void AConnection::cgiSend(std::string msg)
{
	_cgiWriteBuffer += msg;
}
...
void AConnection::onPipePollOut(struct pollfd &pollfd)
{
	...
	if (lenSent == -1)
		std::runtime_error(std::string("AConnection::onPipePollOut(): ") + std::strerror(errno));
...
void AConnection::closeConnection()
{
	Poll::clearPollEvent(POLLIN, this);
}