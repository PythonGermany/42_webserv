...
Address::Address(sa_family_t family, in_port_t port)
{
	...
	{
		oss << _family;
		throw std::invalid_argument("Address::Address(): invalid address family: " + oss.str());
	}
}
...
void Address::family(sa_family_t family)
{
	...
	else
	{
		std::ostringstream oss;
		oss << family;
		throw std::invalid_argument("Address::family(): invalid family: " + oss.str());
	}
}
...
bool Address::operator<(Address const &other) const
{
	...
	if (_family == AF_INET)
	{
		diff_addr = std::memcmp(&_addr.inet.sin_addr, &other._addr.inet.sin_addr, sizeof(_addr.inet.sin_addr));
		diff_port = std::memcmp(&_addr.inet.sin_port, &other._addr.inet.sin_port, sizeof(_addr.inet.sin_port));
	}
	else
	{
		diff_addr = std::memcmp(&_addr.inet6.sin6_addr, &other._addr.inet6.sin6_addr, sizeof(_addr.inet6.sin6_addr));
		diff_port = std::memcmp(&_addr.inet6.sin6_port, &other._addr.inet6.sin6_port, sizeof(_addr.inet6.sin6_port));
	}
	if (diff_addr < 0)
		return true;
	if (diff_port < 0)
...
static sa_family_t splitHost(std::string const &src, std::string &hostname, std::string &port)
{
...
std::set<Address> Address::resolveHost(std::string const &src)
{
	...
	if (ret != 0)
	{
		if (result)
			freeaddrinfo(result);
		if (ret == EAI_SYSTEM)
			throw std::runtime_error("Address::resolveHost: " + src + ": " + std::strerror(errno));
		else
			throw std::runtime_error("Address::resolveHost: " + src + ": " + gai_strerror(ret));
	}
	list = result;
	while (list)
	{
...
bool Address::operator==(Address const &other) const
{
	...
	if (_family == AF_INET)
	{
		if (_addr.inet.sin_port != other._addr.inet.sin_port)
			return false;
		return std::memcmp(&_addr.inet.sin_addr, &other._addr.inet.sin_addr, sizeof(_addr.inet.sin_addr)) == 0;
	}
	else
	{
		if (_addr.inet6.sin6_port != other._addr.inet6.sin6_port)
			return false;
		return std::memcmp(&_addr.inet6.sin6_addr, &other._addr.inet6.sin6_addr, sizeof(_addr.inet6.sin6_addr)) == 0;
	}
}
